...
| **Object → Primitive**      | Objects must be converted to compare values (via `.valueOf()` or `.toString()`),<br>because JavaScript assumes you want to compare the object’s **content**, not its reference.<br> First JS use `.valueOf()`, after this if the value is still bj it's not useful to compare, next it uses `.toString()` to convert to a string |
